      ...
      <Modal v-model="isAddShow" draggable footer-hide scrollable title="增加用户">
        <Form ref="addFormList" :model="addFormList" :rules="ruleValidate" :label-width="80">
          <FormItem label="编号" prop="no">
            <Input v-model="addFormList.no" placeholder="请输入您的编号" number/>
          </FormItem>
          <FormItem label="姓名" prop="name">
            <Input v-model="addFormList.name" placeholder="请输入您的姓名"/>
          </FormItem>
          <FormItem label="邮箱" prop="email">
            <Input v-model="addFormList.email" placeholder="请输入您的邮箱"/>
          </FormItem>
          <FormItem>
...
<script>
  import {
      UserQryAction,UserAddAction,UserDelAction,UserUpdateAction
  } from "../api/user";
  export default {
  name: 'HelloWorld',
  data () {
